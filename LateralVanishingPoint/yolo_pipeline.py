...
--------------File Name: yolo_pipeline.py -----------------------------
Defines the YOLO pipeline for vehicle detection.
...
https://github.com/JunshengFu/vehicle-detection/blob/master/yolo_pipeline.py
...
def vehicle_detection_yolo(image, image_lane, lane_info):
    ...
    #yolo_result,window_list = draw_results1(img)
    yolo_result,window_list = draw_results(image, image_lane, yolo, fps, lane_info)

    return yolo_result,window_list

def vehicle_detection_yolo1(image):
    # set the timer
    #start = timer()
    detect_from_file(yolo, image)
    ...
    #fps = 1.0 / (timer() - start)
    # draw visualization on frame
    window_list = draw_results1(image)
    #yolo_result,window_list = draw_results(image, image_lane, yolo, fps, lane_info)
